...
T_UBYTE Valid_Press(T_UBYTE lub_button)
 {
	T_UBYTE lub_contMs = 0;
	T_UBYTE lub_validPress = 0;
	Restart_STM(TM_1MS);
	do
	{
		if( !Flag_STM(TM_1MS) && !PButt_Press(lub_button) )
		{
			lub_contMs++;
			Restart_STM(TM_1MS);
		}
	}while( !PButt_Press(lub_button) && lub_contMs <= 9);
